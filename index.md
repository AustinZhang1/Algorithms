...
<head>
  ...
  <style>
    ...
    .mainroad, .guestroom, .airconditioning, .parking, .prefarea, .furnishing {
        position: fixed;
        text-align: left;
        left: 70%;
        transform: translate(-50%, -50%);
...
<body>
    ...
    <div class="mainroad">
      ...
      <button id="main-road-yes" class="toggle-button" onclick="togglebutton('main')">Yes</button>
    </div>
    <div class="guestroom">
      <label>Guest Room:</label>
      <div id="guest-room-buttons" class="button-group">
        <button id="guest-room-yes" class="toggle-button" onclick="togglebutton('guest')">Yes</button>
      </div>
    </div>
    <div class="airconditioning">
    <label>Air Conditioning:</label>
    <div id="airconditioning-buttons" class="button-group">
        <button id="airconditioning-yes" class="toggle-button" onclick="togglebutton('airconditioning')">Yes</button>
    </div>
</div>
    <div class="parking">
        <label>Parking:</label>
        <div id="parking-buttons" class="button-group">
            <button id="parking-yes" class="toggle-button" onclick="togglebutton('parking')">Yes</button>
        </div>
    </div>
    <div class="prefarea">
        <label>Preferred Area:</label>
        <div id="pref-area-buttons" class="button-group">
            <button id="pref-area-yes" class="toggle-button" onclick="togglebutton('prefarea')">Yes</button>
        </div>
    </div>
    <div class="furnishing">
        <label>Furnishing Status:</label>
        <div id="furnishing-status-buttons" class="button-group">
            <button id="furnishing-status-yes" class="toggle-button" onclick="togglebutton('furnishing')">Yes</button>
        </div>
    </div>
    <button type="button" onclick="submitForm()">Search</button>

  <script>
    var mainroad = false;guestroom = false;basement = false;hotwater = false;airconditioning = false;parking = false; prefarea = false;furnishing = false;
    ...
    function submitForm() {
      const formData = {
        ...
        parking: parking === true ? 'yes' : 'no',
        prefarea: prefarea === true ? 'yes' : 'no',
        furnishingstatus: furnishing === true ? 'yes' : 'no'
      };
